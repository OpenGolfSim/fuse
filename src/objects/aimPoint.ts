...
export class AimPoint {
  ...
  constructor(camera: THREE.Camera, options: AimPointOptions = {}) {
    ...
    this.ctx = ctx;
    // In constructor, after creating this.ctx:
    this.#panelTexture = new THREE.CanvasTexture(this.canvas);
    this.#panelTexture.colorSpace = THREE.SRGBColorSpace;
    this.#panelTexture.premultiplyAlpha = true;
  ...
  #updateDistanceMaterial(distance: number, height: number, units: string) {
    ...
    this.ctx.fillStyle = colors.background;
    this.ctx.beginPath();
    this.ctx.roundRect(0, 0, this.canvas.width, this.canvas.height, (20 * this.#pixelRatio));
    this.ctx.fill();
    ...
    // Reuse the same texture — just flag it for re-upload
  ...
  update(aimPoint: THREE.Vector3, distance: number, height: number, isShotActive: boolean) {
    ...
    if (this.#cachedTextureKey !== texKey) {
      this.#updateDistanceMaterial(distanceDisplay, heightDisplay, unitsDisplay);
